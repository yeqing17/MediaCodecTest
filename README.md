...
- **v1.1.0**：新增本地文件播放（「文件」按钮 + SAF 选择器，支持 U盘 / sdcard / tmp 任意文件；「本地 media.ts」预设自动扫盘）；修复本地构建（gradle wrapper 8.2 → 8.11.1，对齐 AGP 8.7.3）；给 playurl Config 折叠区加空值兜底，杜绝因布局不一致导致的点击闪退。
- **v1.0.0**：三步 playurl 流程（登录 → 频道信息 → 拼播放地址）、横屏左右分栏布局、VLC UA 绕过直播服务器对 ExoPlayer UA 的限速、live 流按未知长度读取。
